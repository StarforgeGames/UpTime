...
FVector ULevelGeneratorComponent::CalculateTilePosition(int Row, int Column) const
{
	const float Center = (GridSize - 1) * TileSize / 2;
	const float X = Row * TileSize - Center;
	const float Y = Column * TileSize - Center;
...
void ULevelGeneratorComponent::SpawnEnemies(const int NumberOfEnemies, const TSubclassOf<AEnemyCharacter> EnemyType)
{
	...
	{
		const int RandomTile = FMath::RandRange(0, Tiles.Num() - 1);
		const ULevelStreamingDynamic* Tile = Tiles[RandomTile];

		FActorSpawnParameters SpawnParams = FActorSpawnParameters();
		SpawnParams.SpawnCollisionHandlingOverride = ESpawnActorCollisionHandlingMethod::AdjustIfPossibleButAlwaysSpawn;

		FVector RandomLocation = NavSystem->GetRandomReachablePointInRadius(World, Tile->LevelTransform.GetLocation(),
			500);
		FTransform Transform = FTransform(RandomLocation);

		AEnemyCharacter* Enemy = World->SpawnActor<AEnemyCharacter>(EnemyType.GetDefaultObject()->GetClass(), Transform,
			SpawnParams);
		if (Enemy)
		{
...
void ULevelGeneratorComponent::SpawnWeapons(int NumberOfWeapons, TArray<TSubclassOf<APickup>> WeaponPickups)
{
	...
	for (int i = 0; i < NumberOfWeapons; i++)
	{
		...
		const int RandomWeapon = FMath::RandRange(0, WeaponPickups.Num() - 1);
		auto WeaponPickup = WeaponPickups[RandomWeapon];

		const APickup* Pickup = World->SpawnActor<APickup>(WeaponPickup.GetDefaultObject()->GetClass(), Transform,
			SpawnParams);
		if (Pickup)
		{
...
TArray<ULevelStreamingDynamic*> ULevelGeneratorComponent::GetTilesToPopulate() const
{
	...
	// Exclude start tile so no enemies are spawned there
